...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  connect: async () => {
    ...
    newSocket.on(
      "player_joined",
      ({ role, player, whitePlayer, blackPlayer, spectatorCount }: any) => {
    ...
    newSocket.on(
      ...
      (data: { whiteReady: boolean; blackReady: boolean }) => {
  ...
  updateTimerTick: () => {
    const { activeTimerColor, whiteTimeLeft, blackTimeLeft, timerActive } =
      get();
