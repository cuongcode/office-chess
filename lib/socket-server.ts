...
export const initSocketServer = (httpServer: NetServer) => {
    ...
    try {
      ...
      const saved = await prisma.game.create({
        data: {
          ...
          timeControl: timeControlStr,
          whiteTimeLeft: room.timeControl
            ? Math.round(room.whiteTimeLeft)
            : null,
          blackTimeLeft: room.timeControl
            ? Math.round(room.blackTimeLeft)
            : null,
      ...
      console.log(
        `[Server] Game saved: ${saved.id} | result: ${dbResult} | method: ${resultMethod}`,
      );
      return saved.id;
    } catch (err) {
  ...
  const startTimerSync = (roomId: string) => {
    ...
    const interval = setInterval(() => {
      ...
      if (
        !room ||
        !room.timeControl ||
        room.timeControl.category === "unlimited"
      ) {
        return;
      }
  ...
  const handleTimerTimeout = (roomId: string, color: "white" | "black") => {
    ...
    // Update player stats
    const winnerPlayer =
      winner === "white" ? room.whitePlayer : room.blackPlayer;
    const loserPlayer =
      winner === "white" ? room.blackPlayer : room.whitePlayer;
    ...
    console.log(
      `[Server] Emitting game_over for timeout in room ${roomId}, winner: ${winner}`,
    );
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(
      ...
      (data: { roomId: string; userId: string; userName: string }) => {
        ...
        if (
          joinedRoom.whitePlayer &&
          joinedRoom.blackPlayer &&
    ...
    socket.on(
      "make_move",
      (data: { roomId: string; move: any; gameState: any }) => {
        ...
        if (updatedRoom) {
          ...
          if (
            updatedRoom.timeControl &&
            updatedRoom.timeControl.category !== "unlimited"
          ) {
          ...
          if (
            data.gameState.status === "checkmate" &&
            updatedRoom.whitePlayer &&
            updatedRoom.blackPlayer
          ) {
            ...
            (data.gameState.status === "stalemate" ||
              data.gameState.status === "draw") &&
            updatedRoom.whitePlayer &&
            updatedRoom.blackPlayer
          ) {
            updatePlayerStats(updatedRoom.whitePlayer.id, "draw").catch(
              console.error,
            );
            updatePlayerStats(updatedRoom.blackPlayer.id, "draw").catch(
              console.error,
            );
      ...
      if (activeRoom) {
        ...
        if (result.gameJustEnded && result.resignedColor) {
          ...
          const roomForSave = {
            ...activeRoom,
            whitePlayer:
              result.resignedColor === "white"
                ? (result.resignedPlayer ?? null)
                : (result.remainingPlayer ?? null),
            blackPlayer:
              result.resignedColor === "black"
                ? (result.resignedPlayer ?? null)
                : (result.remainingPlayer ?? null),
          };
    ...
    socket.on(
      ...
      (data: { roomId: string; color: "white" | "black" }) => {
        ...
        console.log(
          `[Server] Player ${data.color} is ready in room ${data.roomId}`,
        );
        ...
        if (
          ...
          console.log(
            `[Server] Both players ready, starting timer in room ${data.roomId}`,
          );
