...
export function CapturedPieces({
  ...
  // Calculate points
  const myPoints = capturedPieces.reduce(
    (sum, piece) => sum + (PIECE_VALUES[piece.toLowerCase()] || 0),
    0,
  );
  const opponentPoints = opponentCapturedPieces.reduce(
    (sum, piece) => sum + (PIECE_VALUES[piece.toLowerCase()] || 0),
    0,
  );
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {pointDifference > 0 && (
        <div className="rounded bg-secondary-light px-1.5 py-0.5 text-sm font-bold dark:bg-secondary-dark dark:text-fg-dark">
          +{pointDifference}
        </div>
      )}
      {/* Captured pieces display */}
      <div className="flex min-h-[20px] flex-wrap items-center">
        {[...capturedPieces]
          .sort((a, b) => {
            const valueDiff =
              (PIECE_VALUES[b.toLowerCase()] ?? 0) -
              (PIECE_VALUES[a.toLowerCase()] ?? 0);
          ...
          .map((piece, index) => {
            ...
            const PieceSvg =
              defaultPieces[`w${code}` as keyof typeof defaultPieces];
            ...
            return (
              <span
                ...
                style={{
                  width: 28,
                  height: 28,
                  marginRight: piece.toLowerCase() === "p" ? -16 : -4,
                }}
